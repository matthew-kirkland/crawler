...
const bookmakerString = 'Unibet AU';
...
/**
 * Main scraping function for the Unibet AU bookmaker
 */
export async function unibetAUScraper() {  
  const events = [];
  const bets = [];
  let url, res;
  for (const sport of sports) {
    url = `https://oc-offering-api.kambicdn.com/offering/v2018/ubau/listView/${sport}.json?lang=en_GB&market=ZZ&client_id=2&channel_id=1`;
    ...
    for (const event of res.data.events) {
      ...
      try {
        url = `https://oc-offering-api.kambicdn.com/offering/v2018/ubau/betoffer/event/${event.event.id}.json?lang=en_AU&market=AU&client_id=2&channel_id=1&includeParticipants=true`;
        res = await axios.get(url);
        if (res.status != 200) continue;
